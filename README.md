...
| 层 | 数据来源 | 更新方式 | 真实性保证 |
| --- | --- | --- | --- |
| **每日追踪** | arXiv API（cs.CV / cs.RO）| GitHub Actions 每日 09:23 / 21:23（北京）自动拉取打分，写 `daily.js` 后自动提交 | 条目直接取自 arXiv 元数据，标题可点击溯源 |
...
每日追踪的每篇论文带**中文摘要**（问题 → 方法 → 结果）：

- 由 LLM **严格依据论文英文摘要原文生成**（提示词禁止引入摘要之外的信息），缓存在
  `summaries.json`，同一论文不会重复生成；卡片上明确标注「AI 摘要 · 依原文摘要生成」。
- CI 使用 GitHub Models（`GITHUB_TOKEN` 免费额度，零配置零成本）；调用失败时自动回退为
  显示英文原摘要，不影响数据更新。
- 手动本地生成：编辑 `summaries.json` 后重跑脚本即可，无需 `--llm`。
